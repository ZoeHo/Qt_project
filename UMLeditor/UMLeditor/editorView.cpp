...
{
	ui.setupUi(this);
	setBottom(this);setGraphicsView(this); 
...
{
	sceneView *scene = new sceneView(this);
	mscene=scene;
	QGraphicsView *graphic = new QGraphicsView(scene,this);
...
void editorView::rename()
{
	...
	if(isOK){			//press OK才做 setText
		...
		mscene->mob->getText(text);
		mscene->cob->getText(text);
	}
	}
}
